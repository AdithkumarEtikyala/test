...
export type QuestionStatus = 'answered' | 'not-answered' | 'marked-for-review' | 'not-visited' | 'completed';
...
export interface StudentAnswer {
  ...
  error?: string; // To store execution errors for manual review
...
export const ExecuteCodeInputSchema = z.object({
  ...
  testCases: z.array(TestCaseSchema).optional().describe('An array of test cases to run against the code.'),
});
...
export const ExecuteCodeOutputSchema = z.object({
  ...
  rawOutput: z.string().optional().describe('The raw stdout of a single run if no test cases are provided.'),
  rawError: z.string().optional().describe('The raw stderr of a single run if no test cases are provided.'),
});
