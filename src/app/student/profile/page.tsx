...
import { collection, query, where, getDocs, QuerySnapshot, DocumentData, Timestamp, doc, getDoc } from 'firebase/firestore';
    ...
    const fetchExams = async () => {
      ...
      if (examIds.length > 0) {
        try {
          ...
          // For larger sets, you'd need to chunk the requests.
          const examIdChunks = [];
          for (let i = 0; i < examIds.length; i += 30) {
            examIdChunks.push(examIds.slice(i, i + 30));
          }

          const examsMap = new Map<string, Exam>();
          for (const chunk of examIdChunks) {
            const examsQuery = query(collection(firestore, 'exams'), where('__name__', 'in', chunk));
