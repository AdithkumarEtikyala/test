...
export function ExamInterface({ examId }: { examId: string }) {
  ...
  const handleSubmitExam = useCallback(async (autoSubmitDetails?: { autoSubmitted: boolean, exitCount: number }) => {
    ...
    try {
        ...
        Object.keys(studentExamSubmission).forEach(key => {
            if (studentExamSubmission[key as keyof typeof studentExamSubmission] === undefined) {
                delete studentExamSubmission[key as keyof typeof studentExamSubmission];
            }
        });
  ...
  useEffect(() => {
    if(state && state.timeLeft <= 0 && state.examStarted && !state.examFinished && !isSubmitting) {
        handleAutoSubmit();
    }
